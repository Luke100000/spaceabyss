    ...
    try {
        ...
        if (data.battle_linker) {
            ...
            io.to(data.monster_info.room).emit('damaged_data',
                {
                    'monster_id': dirty.monsters[monster_index].id,
        ...
        if (new_monster_hp <= 0) {
            ...
            if (typeof data.battle_linker !== 'undefined' && data.battle_linker.attacking_type === 'player') {
                await deleteMonster(dirty, monster_index, {
                    'reason': 'battle', 'battle_linker': data.battle_linker,
    ...
    try {
        ...
        if (typeof data.battle_linker === "undefined" && (dirty.monsters[monster_index].monster_type_id === 33 ||
            ...
            let battle_linker_index = dirty.battle_linkers.findIndex(function (obj) {
                return obj && obj.attacking_type === 'monster' && obj.attacking_id == dirty.monsters[monster_index].id;
            });
            ...
            if (drop_linkers.length > 0) {
                ...
                if (drop_success === false) {
                    ...
                    if (monster_info.scope === "planet") {
                        dirty.waiting_drops.push({
                            'object_type_id': drop_linker.dropped_object_type_id,
                            'amount': drop_linker.amount,
                            'planet_coord_index': monster_info.coord_index
                        });
                    } else if (monster_info.scope === "ship") {
                        dirty.waiting_drops.push({
                            'object_type_id': drop_linker.dropped_object_type_id,
                            'amount': drop_linker.amount,
                            'ship_coord_index': monster_info.coord_index
                        });
                    }
        ...
        if ((dirty.monster_types[monster_type_index].id === 33 || dirty.monster_types[monster_type_index].id === 57) && helper.notFalse(data.battle_linker)) {
            ...
            if (monster_info.scope === "planet") {
                let planet_index = await planet.getIndex(dirty, {
                    'planet_id': dirty.planet_coords[monster_info.coord_index].planet_id,
                    'source': 'game.deleteMonster'
                });
        ...
        if (check_spawned_event_id !== 0 && typeof data.skip_check_spawned_event === 'undefined') {
            ...
            let spawned_event_index = dirty.spawned_events.findIndex(function (obj) {
                return obj && obj.id === check_spawned_event_id;
            });
            if (spawned_event_index !== -1 && planet_index !== -1) {
                ...
                let planet_event_linker_index = dirty.planet_event_linkers.findIndex(function (obj) {
                    return obj &&
...
async function getIndex(dirty, monster_id) {
    ...
    try {
        ...
        let monster_index = dirty.monsters.findIndex(function (obj) {
            return obj && obj && obj.id === monster_id;
        });
        ...
        if (monster_index === -1) {
            ...
            try {
                ...
                if (rows[0]) {
                    ...
                    monster_index = dirty.monsters.findIndex(function (obj) {
                        return obj && obj.id === parseInt(monster_id);
                    });
                    if (monster_index !== -1) {
...
async function move(dirty, i, data) {
    ...
    try {
                        ...
                        for (let y = attacking_coord.tile_y - 1;
                            ...
                            if (!found_coord) {
                                ...
                                if (scope === 'planet') {
                                    ...
                                    let checking_data = {
                                        'planet_id': attacking_coord.planet_id,
                                    ...
                                    let checking_data = {
                                        'ship_id': attacking_coord.ship_id,
        ...
        if (!can_place) {
                ...
                if (scope === 'planet' && dirty.planet_coords[new_coord_index].object_id) {
                    ...
                    if (object_index !== -1) {
                        if (dirty.objects[object_index].player_id) {
                            ...
                            await world.addBattleLinker({}, dirty, {
                                'attacking_id': dirty.monsters[i].id, 'attacking_type': 'monster',
            ...
            if (helper.notFalse(attacking_coord) && dirty.monsters[i].planet_coord_id && (typeof dirty.monsters[i].path === "undefined" || dirty.monsters[i].path.length === 0) &&
                ...
                let worker_data = {
                    ...
                    'monster': dirty.monsters[i],
                    'origin_x': dirty.planet_coords[planet_coord_index].tile_x,
                    'origin_y': dirty.planet_coords[planet_coord_index].tile_y,
                    'destination_x': attacking_coord.tile_x,
                    'destination_y': attacking_coord.tile_y
                };
        ...
        if (dirty.monster_types[data.monster_type_index].movement_tile_width > 1 ||
            ...
            for (let ox = old_coord.tile_x; ox <= old_last_x; ox++) {
                for (let oy = old_coord.tile_y; oy <= old_last_y; oy++) {
                    ...
                    if (dirty.monsters[i].planet_coord_id) {
                        let current_planet_coord_index = await main.getPlanetCoordIndex({
                            'tile_x': ox, 'tile_y': oy,
                        ...
                        if (current_planet_coord_index !== -1) {
                            current_coords.push({
                                'planet_coord_id': dirty.planet_coords[current_planet_coord_index].id,
                                'planet_coord_index': current_planet_coord_index, 'still_used': false
                            });
                        }
                    } else if (dirty.monsters[i].ship_coord_id) {
                        let current_ship_coord_index = await main.getShipCoordIndex({
                            'tile_x': ox, 'tile_y': oy,
                        ...
                        if (current_ship_coord_index !== -1) {
                            current_coords.push({
                                'ship_coord_id': dirty.ship_coords[current_ship_coord_index].id,
                        ...
                        if (current_coord_index !== -1) {
                            current_coords.push({
                                'coord_id': dirty.coords[current_coord_index].id,
        ...
        // We just have to push the current coord the monster is on
            ...
            if (dirty.monsters[i].planet_coord_id) {
                current_coords.push({
                    'planet_coord_id': old_coord.id, 'planet_coord_index': old_coord_index,
                    'still_used': false
                });
            } else if (dirty.monsters[i].ship_coord_id) {
                current_coords.push({
                    'ship_coord_id': old_coord.id, 'ship_coord_index': old_coord_index,
                    'still_used': false
                });
            } else if (dirty.monsters[i].coord_id) {
                current_coords.push({
                    'coord_id': old_coord.id, 'coord_index': old_coord_index,
                    'still_used': false
                });
            }
        ...
        for (let x = new_x; x <= last_x; x++) {
            for (let y = new_y; y <= last_y; y++) {
                ...
                if (dirty.monsters[i].planet_coord_id) {
                    current_coord_index = current_coords.findIndex(function (obj) {
                        return obj &&
        ...
        if (dropping_coord_index !== -1) {
            if (scope === 'planet') {
                ...
                await main.updateCoordGeneric(false, {
                    'planet_coord_index': dropping_coord_index, 'object_type_id': drop_linker.dropped_object_type_id,
                    'amount': drop_linker.amount
                });
            } else if (scope === 'ship') {
                //console.log("Monster is dropping amount: " + drop_linker.amount + " of object_type_id: " + drop_linker.dropped_object_type_id);
                await main.updateCoordGeneric(false, {
                    'ship_coord_index': dropping_coord_index, 'object_type_id': drop_linker.dropped_object_type_id,
                    'amount': drop_linker.amount
                });
            } else if (scope === 'galaxy') {
                await main.updateCoordGeneric(false, {
                    'coord_index': dropping_coord_index, 'object_type_id': drop_linker.dropped_object_type_id,
                    'amount': drop_linker.amount
                });
            }
    ...
    try {
        ...
        // sent in an ID rather than an index
            ...
            let planet_coord_index = dirty.planet_coords.findIndex(function (obj) {
                return obj && obj.monster_id === data.monster_id;
            });
            if (planet_coord_index !== -1) {
...
async function spawn(dirty, monster_type_id, data) {
    ...
    try {
        ...
        if (scope === 'planet') {
            ...
            await main.updateCoordGeneric(false, {
                'ship_coord_index': coord_index, 'monster_id': new_monster_id,
                'spawned_monster_id': new_monster_id
            });
        } else if (scope === 'galaxy') {
            ...
            if (data.attack_on_spawn_type === 'player' &&
                ...
                if (being_attacked_player_index !== -1) {
                    ...
                    if (being_attacked_player_socket) {
                        being_attacked_player_socket.emit('chat', {
                            'message': "You have violated the rules of the ruling AI.",
                            'scope': 'local',
                            'is_important': true
                        });
                    }
