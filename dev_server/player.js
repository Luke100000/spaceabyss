    ...
    try {
                ...
                if (object_type_index !== -1) {
                    ...
                    if (dirty.object_types[object_type_index].min_attack_range <= calculating_range && dirty.object_types[object_type_index].max_attack_range >= calculating_range) {
                        ...
                        if (dirty.object_types[object_type_index].attack_radius) {
                            radius_damage_amounts.push({
                                'radius': dirty.object_types[object_type_index].attack_radius,
                                'damage_amount': dirty.object_types[object_type_index].attack_strength,
        ...
        for (let eating_linker of body_eating_linkers) {
            if (eating_linker && eating_linker.body_id === dirty.players[player_index].body_id) {
                ...
                let race_linker_index = main.getRaceEatingLinkerIndex({
                    'race_id': dirty.object_types[player_body_type_index].race_id,
                    'object_type_id': eating_linker.eating_object_type_id
                });
        ...
        for (let addiction_linker of body_addiction_linkers) {
            ...
            if (!still_eating) {
                ...
                let race_linker_index = main.getRaceEatingLinkerIndex({
                    'race_id': dirty.object_types[player_body_type_index].race_id,
                    'object_type_id': addiction_linker.addicted_to_object_type_id
                });
        ...
        return {
            'damage_amount': damage_amount, 'damage_types': damage_types,
...
async function calculateDefense(dirty, player_index, damage_type = '') {
    ...
    try {
        ...
        for (let equipment_linker of player_equipment_linkers) {
            let object_type_index = dirty.object_types.findIndex(function (obj) {
                return obj && obj && obj.id === equipment_linker.object_type_id;
            });
            if (object_type_index !== -1) {
                // see if the object attack is in this range
        ...
        for (let i = 0; i < dirty.eating_linkers.length; i++) {
            if (dirty.eating_linkers[i] && dirty.eating_linkers[i].body_id === dirty.players[player_index].body_id) {
                let race_linker_index = main.getRaceEatingLinkerIndex({
                    'race_id': dirty.object_types[player_body_type_index].race_id,
                    'object_type_id': dirty.eating_linkers[i].eating_object_type_id
                });
            ...
            for (let x = coord.tile_x; x <= last_x; x++) {
                for (let y = coord.tile_y; y <= last_y; y++) {
                    ...
                    if (scope === 'galaxy') {
                        ...
                        checking_coord_index = await main.getPlanetCoordIndex({
    ...
    try {
        ...
        if (!dirty.object_types[ship_object_type_index].can_be_claimed) {
            ...
            socket.emit('result_info', {
                'status': 'failure',
                'text': dirty.object_types[ship_object_type_index].name + " will not let you claim it"
            });
            return false;
        }
            ...
            if (data.battle_linker) {
                if (helper.notFalse(player_socket)) {
                    io.sockets.connected[data.battle_linker.being_attacked_socket_id].emit('damaged_data', {
                        'player_id': dirty.players[data.player_index].id,
        ...
        if (typeof data.damage_types !== 'undefined' && data.damage_types.length !== 0) {
            for (let i = 0; i < data.damage_types.length; i++) {
                ...
                if (data.damage_types[i] === 'poison') {
                    ...
                    game.addAddictionLinker(dirty, player_socket, object_type_index, {
                        'player_index': data.player_index,
                        'player_body_index': player_body_index
                    });
                }
            }
        }
        ...
        if (rows[0]) {
            for (let i = 0; i < rows.length; i++) {
                ...
                let equipment_linker_index = dirty.equipment_linkers.findIndex(function (obj) {
                    return obj && obj.id === equipment_linker.id
                });
...
async function getIndex(dirty, data) {
    ...
    try {
        ...
        if (data.player_id) {
            player_index = dirty.players.findIndex(function (obj) {
                return obj && obj.id === data.player_id;
            });
        }
        ...
        // dirty isn't written yet)
            ...
            player_index = dirty.players.findIndex(function (obj) {
                return obj && obj.body_id === data.body_id;
            });
        } else if (data.name) {
        ...
        if (player_index === -1) {
            ...
            if (rows[0]) {
                ...
                player_index = dirty.players.findIndex(function (obj) {
                    return obj && obj.id === adding_player.id;
                });
        ...
        if (body_index !== -1) {
            ...
            for (let i = 0; i < dirty.eating_linkers.length; i++) {
                if (dirty.eating_linkers[i] && dirty.eating_linkers[i].body_id === dirty.objects[body_index].id) {
                    let race_linker_index = main.getRaceEatingLinkerIndex({
                        'race_id': dirty.object_types[body_type_index].race_id,
                        'object_type_id': dirty.eating_linkers[i].eating_object_type_id
                    });
            ...
            for (let i = 0; i < dirty.addiction_linkers.length; i++) {
                ...
                if (dirty.addiction_linkers[i] && dirty.addiction_linkers[i].body_id === dirty.objects[body_index].id) {
                    ...
                    if (!still_eating) {
                        let race_linker_index = main.getRaceEatingLinkerIndex({
                            'race_id': dirty.object_types[body_type_index].race_id,
                            'object_type_id': dirty.addiction_linkers[i].addicted_to_object_type_id
                        });
    ...
    try {
        ...
        if (rows[0]) {
            for (let i = 0; i < rows.length; i++) {
                ...
                let player_relationship_linker_index = dirty.player_relationship_linkers.findIndex(function (obj) {
                    return obj && obj.id === player_relationship_linker.id;
                });
        ...
        if (rows[0]) {
            for (let i = 0; i < rows.length; i++) {
                ...
                let player_research_linker_index = dirty.player_research_linkers.findIndex(function (obj) {
                    return obj && obj.id === player_research_linker.id
                });
...
async function kill(dirty, player_index, killed_by_text = "") {
    ...
    try {
        ...
        let database_queue_index = dirty.database_queue.findIndex(function (obj) {
            return obj &&
        ...
        // Ship ID before coord id
            ...
            if (!dirty.ship_coords[coord_index].object_id) {
                ...
                await main.updateCoordGeneric(player_socket, {
        ...
        for (let i = 0; i < dirty.inventory_items.length; i++) {
            if (dirty.inventory_items[i] && dirty.inventory_items[i].player_id === dirty.players[player_index].id && dirty.inventory_items[i].body_id === dirty.objects[body_index].id) {
                ...
                let remove_data = {
                    ...
                    'amount': dirty.inventory_items[i].amount
                };
                let add_data = {
                    'adding_to_type': 'object',
                    'adding_to_id': dirty.objects[new_object_index].id,
                    'amount': dirty.inventory_items[i].amount
                };
    ...
    try {
            ...
            if (!dirty.objects[old_ship_index].is_dockable && dirty.objects[new_ship_index].object_type_id === 114 &&
                ...
                if (old_ship_coord_index !== -1) {
                    main.updateCoordGeneric(socket, {
                        'coord_index': old_ship_coord_index,
                        'object_id': dirty.objects[old_ship_index].id
                    });
                }
        ...
        // If we are on the galaxy, we are moving the player to the coord with the new ship
            ...
            await main.updateCoordGeneric(socket, {
                'coord_index': old_player_coord_index,
                'object_id': dirty.objects[old_ship_index].id, 'player_id': false
            });

            // And moves to the new ship
            await main.updateCoordGeneric(socket, {
                'coord_index': new_ship_coord_index, 'object_id': false,
                'player_id': dirty.players[player_index].id
            });
...
async function unequip(socket, dirty, equipment_linker_id) {
    ...
    try {
        ...
        let equipment_linker_index = dirty.equipment_linkers.findIndex(function (obj) {
            return obj && obj.id === equipment_linker_id;
        });
        ...
        if (dirty.equipment_linkers[equipment_linker_index].object_id) {
            ...
            let adding_to_data = {
                ...
                'object_type_id': dirty.equipment_linkers[equipment_linker_index].object_type_id,
                'amount': dirty.equipment_linkers[equipment_linker_index].amount
            };
            await inventory.addToInventory(socket, dirty, adding_to_data);
        }
        ...
        socket.emit('equipment_linker_info', {
            'remove': true,
            'equipment_linker': dirty.equipment_linkers[equipment_linker_index]
        });
