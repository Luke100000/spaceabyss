            ...
            try {
                ...
                if ((data.is_forced && data.is_forced === true) || can_place_object_result) {
                    ...
                    if (data.is_forced && data.is_forced === true) {
                        ...
                        if (dirty.planet_coords[checking_coord_index].object_id) {
                            ...
                            if (object_index !== -1) {
                                await game_object.deleteObject(dirty, {
                                    'object_index': object_index,
                                    'reason': "Forced in game.buildStructure"
                                });
                            }
                    ...
                    if (structure_linker.area_name) {
                        ...
                        let area_index = dirty.areas.findIndex(function (obj) {
                            return obj && obj.owner_id === 4 && obj.name === structure_linker.area_name;
                        });
                ...
                if (structure_linker.place_npc === true && typeof data.npc_index !== 'undefined' && data.npc_index !== -1) {
                    ...
                    await main.updateCoordGeneric(false, {
                        'planet_coord_index': new_npc_coord_index,
                        'npc_id': dirty.npcs[data.npc_index].id
                    });
...
async function placeAI(socket, dirty, data) {
    try {
        ...
        if (data.planet_coord_index) {
            ...
            planet_index = await planet.getIndex(dirty, {
                'planet_id': dirty.planet_coords[data.planet_coord_index].planet_id,
                'source': 'game.placeAI'
            });
        ...
        if (data.planet_coord_index) {
            ...
            await main.updateCoordGeneric(socket, {
                'ship_coord_index': data.ship_coord_index,
                'object_index': object_index
            });
...
async function placePortal(socket, dirty, object_index, data) {
    ...
    try {
        ...
        if (helper.notFalse(dirty.objects[object_index].player_id)) {
            ...
            unattached_portal_index = dirty.objects.findIndex(function (obj) {
        ...
        // Otherwise, it's possible that our object has a spawned_event_id, and we can try to link based on that
            ...
            unattached_portal_index = dirty.objects.findIndex(function (obj) {
                return obj && obj.spawned_event_id === dirty.objects[object_index].spawned_event_id &&
                    obj.object_type_id === dirty.object_types[object_type_index].attaches_to_object_type_id && !obj.attached_to_id && obj.id !== dirty.objects[object_index].id;
            });
        }
...
async function addAddictionLinker(dirty, socket, object_type_index, data) {
    ...
    try {
        ...
        let addiction_linker_index = dirty.addiction_linkers.findIndex(function (obj) {
            return obj &&
...
async function addPortal(socket, dirty, data) {
    ...
    try {
        ...
        let unattached_portal_index = dirty.objects.findIndex(function (obj) {
            return obj &&
...
async function assemble(socket, dirty, data) {
    ...
    try {
        ...
        if (data.object_type_id) {
            ...
            if (!requirements_met) {
                socket.emit('chat', {
                    'message': "Unable to assemble. Did not meet assembly requirements",
                    'scope': 'system'
                });
                return false;
            }
            ...
            if (dirty.object_types[object_type_index].assembled_in_object) {
                ...
                let assembled_in_linker_index = dirty.assembled_in_linkers.findIndex(function (obj) {
                    return obj &&
            ...
            let floor_type_index = dirty.floor_types.findIndex(function (obj) {
                return obj && obj.id === parseInt(data.floor_type_id);
            });
...
async function convert(socket, dirty, data) {
    try {
        ...
        if (using_conversion_linker.output_destination === 'user_inventory') {
            ...
            // Delete ourself
            await game_object.deleteObject(dirty, {
                'object_index': converter_object_index,
                'reason': 'game.convert - output replaces us'
            });
            ...
            if (using_conversion_linker.output_object_type_id) {
                ...
                if (dirty.object_types[output_object_type_index].assembled_as_object) {
                    let insert_object_type_data = {
                        'object_type_id': using_conversion_linker.output_object_type_id,
                        'source': 'game.convert'
                    };
                    let new_object_index = await world.insertObjectType(false, dirty, insert_object_type_data);
                    ...
                    if (converter_info.scope === "planet") {
                        await game_object.place(socket, dirty, {
                            'object_index': new_object_index,
                            'planet_coord_index': converter_info.coord_index
                        });
                    } else if (converter_info.scope === "ship") {
                        await game_object.place(socket, dirty, {
                            'object_index': new_object_index,
                            'ship_coord_index': converter_info.coord_index
                        });
                    }
            ...
            let success = false;
            if (socket) {
                farming_level = await player.getLevel(dirty, {
            ...
            if (!success) {
                ...
                if (socket) {
                    socket.emit('chat', {
                        'message': 'Your attempt to help has failed, and you have damaged it instead',
                        'scope': 'system'
                    });
                }
                ...
                if (socket) {
                    ...
                    socket.emit('result_info', {
                        'status': 'failure', 'text': "You botched the job",
                ...
                if (new_converter_object_hp > dirty.object_types[converter_object_type_index].hp + level_difference) {
                    ...
                    if (socket) {
                        ...
                        socket.emit('result_info', {
                            'status': 'success', 'text': "As Healthy As Your Skills Can Make It!",
                            'object_id': dirty.objects[converter_object_index].id
                        });
                    }
                    ...
                    if (socket) {
                        ...
                        socket.emit('result_info', {
                            'status': 'success', 'text': "+" + using_conversion_linker.output_amount + "HP",
                            'object_id': dirty.objects[converter_object_index].id
                        });
                    }
...
async function createArea(socket, dirty, new_area_name, player_index = -1, auto_market = false) {
    ...
    try {
        ...
        let existing_area_index = dirty.areas.findIndex(function (obj) {
            return obj && obj.name === new_area_name
        });
            ...
            io.to(data.planet_info.room).emit('damaged_data',
                {
                    ...
                    'was_damaged_type': 'hp',
                    'damage_source_type': data.battle_linker.attacking_type,
                    'damage_source_id': data.battle_linker.attacking_id,
            ...
            io.to(data.planet_info.room).emit('damaged_data',
                {
                    ...
                    'damage_source_id': data.damage_source_id,
                    'damage_types': data.damage_types,
                    'calculating_range': data.calculating_range
                });
        } else {
    ...
    try {
        ...
        for (let i = 0; i < dirty.monsters.length; i++) {
            ...
            if (dirty.monsters[i] && dirty.monsters[i].monster_type_id === dirty.monster_types[monster_type_index].id) {
                await monster.damage(dirty, i, dirty.monster_types[monster_type_index].decay_rate, {
                    'damage_source_type': 'decay',
                    'damage_types': ['decay']
                });
            }
...
async function eat(socket, dirty, data) {
    ...
    try {
        ...
        if (socket) {
            race_eating_linker_index = dirty.race_eating_linkers.findIndex(function (obj) {
                return obj &&
        ...
        if (socket) {
            database_queue_index = dirty.database_queue.findIndex(function (obj) {
                return obj &&
        ...
        if (socket) {
            database_queue_index = dirty.database_queue.push({
                'player_id': socket.player_id,
                'eating_object_type_id': dirty.object_types[inventory_item_object_type_index].id
            }) - 1;
        } else {
            database_queue_index = dirty.database_queue.push({
    ...
    try {
        ...
        if (rows[0]) {
            ...
            for (let i = 0; i < rows.length; i++) {
                ...
                let final_check_index = dirty.planet_coords.findIndex(function (obj) {
                    return obj && obj.id === adding_planet_coord.id;
                });
        ...
        if (data.object_id) {
            ...
            if (object_index === -1) {
                ...
                let planet_coord_index = dirty.planet_coords.findIndex(function (obj) {
                    return obj && obj.object_id === data.object_id;
                });
                if (planet_coord_index !== -1) {
                    await main.updateCoordGeneric(dirty, {
    ...
    try {
        ...
        if (dirty.players[player_index].planet_coord_id) {
            ...
            if (dirty.inventory_items[inventory_item_index].object_id && dirty.objects[object_index].object_type_id === 116) {
                socket.emit('chat', {
                    'message': "Can't place shipyards on planets. Shipyards must be built in space",
                    'scope': 'system'
                });
            ...
            let planet_coord_data = {
                'planet_id': dirty.planet_coords[player_planet_coord_index].planet_id,
            ...
            if (dirty.inventory_items[inventory_item_index].object_id && dirty.objects[object_index].object_type_id === 116) {
                socket.emit('chat', {
                    'message': "Can't place shipyards on other ships. Shipyards must be built in space",
                    'scope': 'system'
                });
                return false;
            }
            ...
            let ship_coord_data = {
                'ship_id': dirty.ship_coords[player_ship_coord_index].ship_id,
        ...
        if (dirty.object_types[object_type_index].drop_requires_object_type_id) {
            if (temp_coord.object_type_id === dirty.object_types[object_type_index].drop_requires_object_type_id) {
                ...
                socket.emit('result_info', {
                    'status': 'failure',
                    'text': "Must place on " + dirty.object_types[required_object_type_index].name
                });
        ...
        if (helper.notFalse(dirty.object_types[object_type_index].drop_requires_floor_type_class)) {
            ...
            if (floor_type_index === -1 || dirty.floor_types[floor_type_index].class !== dirty.object_types[object_type_index].drop_requires_floor_type_class) {
                socket.emit('result_info', {
                    'status': 'failure',
                    'text': "Must place on " + dirty.object_types[object_type_index].drop_requires_floor_type_class + " floor"
                });
        ...
        if (temp_coord.object_type_id && !temp_coord.object_id &&
            ...
            if (dirty.inventory_items[inventory_item_index].object_id) {
                ...
                if (scope === 'planet') {
                    let planet_index = await planet.getIndex(dirty, {
                        'planet_id': temp_coord.planet_id,
                        'source': 'game.drop'
                    });
                        ...
                        for (let ai_rule of ai_rules) {
                            ...
                            if (ai_rule.rule === 'building_no_others') {
                                if (socket.player_id !== dirty.objects[ai_index].player_id) {
                                    ...
                                    socket.emit('result_info', {
                                        'status': 'failure',
                                        'text': "The AI has prevented this action"
                                    });
                                    socket.emit('chat', {
                                        'message': "The AI does not allow you to build here",
                                        'scope': 'system'
                                    });
                                    return false;
                                }
                ...
                let spawn_linker_index = dirty.spawn_linkers.findIndex(function (obj) {
                    return obj && obj.object_type_id === dirty.objects[object_index].object_type_id;
                });
...
async function equipItem(socket, dirty, data) {
    ...
    try {
        ...
        dirty.equipment_linkers.forEach(function (equipment_linker, i) {
            if (equipment_linker.body_id === dirty.players[socket.player_index].body_id && equipment_linker.equip_slot === data.equip_slot) {
                ...
                let existing_equipment_linker_index = main.getObjectTypeEquipmentLinkerIndex(
                    {
                        'object_type_id': dirty.object_types[existing_object_type_index].id,
                        'equip_slot': equipment_linker.equip_slot
                    });
        ...
        if (maximum_capacity !== 0 && current_capacity_used + dirty.object_type_equipment_linkers[equipment_linker_index].capacity_used > maximum_capacity) {
            socket.emit('chat', {
                'message': "Unable to equip. Slot does not hae enough capacity to equip this.",
                'scope': 'system'
            });
            return false;
        }
        ...
        if (current_equip_slot_count > 0 || dirty.object_type_equipment_linkers[equipment_linker_index].auto_doc_required) {
            ...
            if (player_info.coord.object_type_id !== 140) {
                ...
                socket.emit('chat', {
                    'message': "Equipping this at this location requires an auto doc. You need to be standing on an auto doc",
                    'scope': 'system'
                });
                    ...
                    for (let i = 0; i < dirty.inventory_items.length && paid_cost === false; i++) {
                        ...
                        if (dirty.inventory_items[i] && dirty.inventory_items[i].player_id === dirty.players[socket.player_index].id && dirty.inventory_items[i].object_type_id === 74) {
                            ...
                            if (dirty.inventory_items[i].amount >= surgery_level) {
                                ...
                                // remove from us
                                let remove_inventory_data = {
                                    'inventory_item_id': dirty.inventory_items[i].id,
                                    'amount': surgery_level
                                };
                                await inventory.removeFromInventory(socket, dirty, remove_inventory_data);
                                paid_cost = true;
                                ...
                                let adding_to_data = {
                                    'adding_to_type': 'npc', 'adding_to_id': dirty.npcs[npc_index].id,
                            ...
                            // Failed to pay for it
                                ...
                                socket.emit('chat', {
                                    'message': "Failed to pay credit cost of " + surgery_level,
                                    'scope': 'system'
                                });
            ...
            if (success === false) {
                ...
                dirty.equipment_linkers.forEach(await async function (equipment_linker, i) {
                    ...
                    try {
                        if (equipment_linker.body_id === dirty.players[socket.player_index].body_id && equipment_linker.equip_slot === data.equip_slot) {
                            ...
                            socket.emit('equipment_linker_info', {
                                'equipment_linker': equipment_linker,
                                'remove': true
                            });
                ...
                await player.damage(dirty, {
                    'player_index': socket.player_index, 'damage_amount': resulting_complexity,
                ...
                socket.emit('chat', {
                    'message': "Equipping item failed. Everything in the slot was destroyed",
                    'scope': 'system'
                });
    ...
    try {
        ...
        if (dirty.spawn_linkers[data.spawn_linker_index].spawns_location === 'has_spawned_object') {
            ...
            if (helper.notFalse(dirty.spawn_linkers[data.spawn_linker_index].spawns_object_type_id)) {
                ...
                let insert_object_type_data = {
                    'object_type_id': dirty.spawn_linkers[data.spawn_linker_index].spawns_object_type_id,
            ...
            for (let i = 0; i < dirty.ship_coords.length && coords_to_damage > 0; i++) {
                ...
                if (dirty.ship_coords[i] && dirty.ship_coords[i].ship_id === dirty.objects[ship_index].id) {
                    ...
                    if (dirty.ship_coords[i].object_type_id) {
                        ...
                        if (object_type_index !== -1) {
                            ...
                            if (dirty.object_types[object_type_index].is_ship_wall) {
                                ...
                                let turns_into_object_type_index = dirty.object_types.findIndex(function (obj) {
                                    return obj &&
                    ...
                    if (!is_ship_wall_coord) {
                        ...
                        let turns_into_floor_type_index = dirty.floor_types.findIndex(function (obj) {
                            return obj &&
...
async function mine(socket, dirty, data) {
    try {
        ...
        let mining_index = dirty.mining_linkers.findIndex(function (obj) {
            return obj &&
...
async function mineStop(socket, dirty, data) {
    try {
        ...
        if (data.mining_linker_id) {
            mining_linker_index = dirty.mining_linkers.findIndex(function (obj) {
                return obj && obj.id === data.mining_linker_id;
            })
        ...
        if (mining_linker_index !== -1) {
            ...
            socket.emit('mining_linker_info', {
                'remove': true,
                'mining_linker': dirty.mining_linkers[mining_linker_index]
            });
...
async function deletePlanetCoord(dirty, data) {
    ...
    try {
        ...
        await movement.warpTo(socket, dirty, {
            'player_id': dirty.planet_coords[data.planet_coord_index].player_id,
            'planet_id': dirty.planet_coords[data.planet_coord_index].planet_id, 'warping_to': 'spaceport '
        });

        // Npcs are moved to Spaceport
        await movement.warpTo(socket, dirty, {
            'npc_id': dirty.planet_coords[data.planet_coord_index].npc_id,
            'planet_id': dirty.planet_coords[data.planet_coord_index].planet_id, 'warping_to': 'spaceport '
        });
...
async function deleteShip(socket, dirty, ship_id, reason) {
    ...
    try {
        ...
        if (reason !== 'switchship' && dirty.players[player_index].ship_id === ship_id) {
            ...
            let new_ship_index = await world.insertObjectType(false, dirty, {
                'object_type_id': 114,
...
async function metStructureRequirements(dirty, npc_index) {
    ...
    requirements.forEach(function (requirement, i) {
        ...
        let npc_inventory_item_index = dirty.inventory_items.findIndex(function (obj) {
            return obj && obj.npc_id === dirty.npcs[npc_index].id
        ...
        if (object_type_spawn_linkers.length > 0) {
            ...
            if (met_requirements) {
                ...
                // We finish it
                    ...
                    let spawn_linker_index = dirty.spawn_linkers.findIndex(function (obj) {
                        return obj && obj.id === chosen_linker.id;
                    });
                    await finishSpawnLinker(dirty, {
                        'object_index': object_index,
                        'spawn_linker_index': spawn_linker_index
                    });
...
async function processAddictionLinker(dirty, i) {
    try {
        ...
        let race_eating_index = dirty.race_eating_linkers.findIndex(function (obj) {
            return obj &&
        ...
        if (race_eating_index === -1) {
            ...
            if (helper.notFalse(player_socket)) {
                player_socket.emit('addiction_linker_info', {
                    'remove': true,
                    'addiction_linker': dirty.addiction_linkers[i]
                });
            }
            await (pool.query("DELETE FROM addiction_linkers WHERE id = ?", [dirty.addiction_linkers[i].id]));
            delete dirty.addiction_linkers[i];
                        ...
                        if (new_player_hp <= 0) {
                            ...
                            if (helper.notFalse(player_socket)) {
                                player_socket.emit('damaged_data', {
                                    'player_id': dirty.players[player_index].id,
                                    'damage_amount': addiction_damage_amount,
                                    'was_damaged_type': 'hp',
        ...
        if (data.message.includes("/addinventory")) {
            ...
            if (dirty.object_types[object_type_index].assembled_as_object) {
                ...
                for (let x = 1; x <= amount; x++) {
                    ...
                    await inventory.addToInventory(socket, dirty, {
                        'adding_to_type': 'player', 'adding_to_id': socket.player_id,
            ...
            await buildStructure(dirty, {
                'structure_type_id': structure_type_id,
                'is_forced': true,
                'starting_tile_x': dirty.planet_coords[planet_coord_index].tile_x,
                'starting_tile_y': dirty.planet_coords[planet_coord_index].tile_y,
                'starting_level': dirty.planet_coords[planet_coord_index].level,
            ...
            await world.connectLevel(socket, dirty, {
                'planet_index': planet_index,
                'planet_type_index': planet_type_index,
                'connecting_level': level
            });
            ...
            let spawned_event_index = dirty.spawned_events.findIndex(function (obj) {
                return obj && obj.id === spawned_event_id;
            });
            ...
            for (let x = dirty.planet_coords[player_info.coord_index].tile_x - 2; x <= dirty.planet_coords[player_info.coord_index].tile_x + 2; x++) {
                for (let y = dirty.planet_coords[player_info.coord_index].tile_y - 2; y <= dirty.planet_coords[player_info.coord_index].tile_y + 2; y++) {
                    ...
                    if (found_coord_index === -1) {
                        let possible_coord_index = await main.getPlanetCoordIndex({
                            'planet_id': dirty.planet_coords[player_info.coord_index].planet_id,
            ...
            await main.updateCoordGeneric(socket, {
                'planet_coord_index': found_coord_index,
                'player_id': dirty.players[player_index].id
            });
            ...
            await world.setPlanetType(socket, dirty, {
                'planet_index': planet_index,
                'planet_type_index': planet_type_index
            });
            ...
            if (dirty.players[socket.player_index].planet_coord_id) {
                ...
                for (let x = dirty.planet_coords[coord_index].tile_x - 1; x <= dirty.planet_coords[coord_index].tile_x + 1; x++) {
                    for (let y = dirty.planet_coords[coord_index].tile_y - 1; y <= dirty.planet_coords[coord_index].tile_y + 1; y++) {
                        ...
                        if (found_coord_index === -1) {
                            let checking_coord_index = await main.getPlanetCoordIndex({
            ...
            let npc_job_index = dirty.npc_jobs.findIndex(function (obj) {
                return obj && obj.id === parseInt(npc_job_id);
            });
            ...
            if (dirty.object_types[object_type_index].assembled_as_object) {
                ...
                if (dirty.players[player_index].ship_coord_id) {
                    ...
                    await game_object.place(socket, dirty, {
                        'object_index': new_object_index,
                        'ship_coord_index': found_coord_index
                    });
                } else if (dirty.players[player_index].coord_id) {
    ...
    try {
        ...
        if (data.scope === "system") {
            ...
            io.emit('chat', {
                ...
                'scope': data.scope,
                'source_type': 'player',
                'source_id': dirty.players[sending_player_index].id
            });
        } else {
    ...
    try {
        ...
        if (player_info.room === object_info.room) {
            ...
            if (x_difference > 2 || y_difference > 2) {
                ...
                io.to(dirty.mining_linkers[i].player_socket_id).emit('chat', {
                    'message': "Too far away from mining object",
                    'scope': 'system'
                });
        ...
        let spawn_linker_index = dirty.spawn_linkers.findIndex(function (obj) {
            return obj && obj.id === dirty.objects[object_index].current_spawn_linker_id;
        });
        ...
        mining_socket.emit('result_info', {
            'status': 'success',
...
async function processRepairingLinker(dirty, i) {
    ...
    try {
        ...
        if (inventory_item_index === -1) {
            ...
            io.sockets.connected[dirty.repairing_linkers[i].player_socket_id].emit('result_info', {
                'status': 'failure',
        ...
        // COMPLEXITY CHECK
        let repair_level = await player.getLevel(dirty, {
            'player_index': dirty.repairing_linkers[i].player_index,
            'skill_type': 'repairing'
        });
        ...
        io.sockets.connected[dirty.repairing_linkers[i].player_socket_id].emit('repair_info', {
            'repairing_linker_id': dirty.repairing_linkers[i].id,
            'repaired_amount': repair_level
        });
    ...
    try {
        ...
        if (player_info.room === salvaging_room) {
            ...
            if (x_difference > 2 || y_difference > 2) {
                ...
                io.to(dirty.active_salvagings[i].player_socket_id).emit('chat', {
                    'message': "Too far away from salvaging object",
                    'scope': 'system'
                });
        ...
        let salvaging_level = await player.getLevel(dirty, {
            'player_index': dirty.active_salvagings[i].player_index,
                    ...
                    if (dirty.object_types[salvaged_object_type_index].assembled_as_object) {
                        ...
                        for (let x = 1; x <= chosen_salvage_linker.amount; x++) {
                            ...
                            if (salvaging_room === 'galaxy') {
                                ...
                                let insert_object_type_data = {
                                    'object_type_id': dirty.object_types[salvaged_object_type_index].id,
                                    'source': 'game.salvage'
                                };
                                let new_object_index = await world.insertObjectType(socket, dirty, insert_object_type_data);
                        ...
                        if (salvaging_room === 'galaxy') {
                            ...
                            await inventory.addToInventory(socket, dirty, {
                                'adding_to_type': 'player',
                                'adding_to_id': socket.player_id,
                                'object_type_id': dirty.object_types[salvaged_object_type_index].id,
                                'amount': chosen_salvage_linker.amount
                            });
                        }
...
async function salvage(socket, dirty, data) {
    try {
        ...
        if (typeof data.object_id !== 'undefined') {
            ...
            let salvaging_index = dirty.active_salvagings.findIndex(function (obj) {
                return obj &&
                    obj.player_id === socket.player_id && obj.object_id === dirty.objects[object_index].id;
            });
...
async function salvageStop(socket, dirty, data) {
    try {
        ...
        let active_salvaging_index = dirty.active_salvagings.findIndex(function (obj) {
            return obj && obj.id === data.salvaging_linker_id;
        });
...
async function setAutopilotDestination(socket, dirty, destination_tile_x, destination_tile_y) {
    ...
    try {
        ...
        let autopilot_index = dirty.autopilots.push({
            'id': uuidv1(), 'player_id': socket.player_id, 'destination_tile_x': parseInt(destination_tile_x),
            'destination_tile_y': parseInt(destination_tile_y)
        }) - 1;
        ...
        dirty.assemblies.forEach(await async function (assembly, i) {
            ...
            try {
                ...
                if (dirty.assemblies[i].amount_completed >= dirty.assemblies[i].total_amount) {
                    ...
                    io.to(assembler_object_info.room).emit('assembly_info', {
                        'assembly': dirty.assemblies[i],
                        'finished': true
                    });
                ...
                // Manufacturing
                    ...
                    level = await player.getLevel(dirty, {
                        'player_index': player_index,
                        'skill_type': 'manufacturing',
                        'scope': assembler_object_info.scope,
                ...
                if (success) {
                    ...
                    if (!dirty.object_types[assembled_object_type_index].assembled_as_object) {
                        let adding_to_data = {
                            'adding_to_type': 'object', 'adding_to_id': assembly.assembler_object_id,
                        ...
                        let object_data = {
                            'object_type_id': assembly.being_assembled_object_type_id,
                    ...
                    if (helper.notFalse(player_socket)) {
                        player_socket.emit('result_info', {
                            'status': 'success',
                    ...
                    /**************** DIDN'T ASSEMBLE A SHIP ************************/
                        ...
                        if (dirty.object_types[assembled_object_type_index].assembled_as_object) {
                            ...
                            let adding_to_data = {
                                'adding_to_type': 'object',
                                'adding_to_id': assembly.assembler_object_id,
                                'amount': 1,
                                'object_id': dirty.objects[new_object_index].id,
                                'object_type_id': assembly.being_assembled_object_type_id
                            };
                        ...
                        if (dirty.players[player_index].socket_id) {
                            ...
                            if (helper.notFalse(player_socket)) {
                                ...
                                player_socket.emit('assembly_info', {
                                    'finished': true,
                                    'assembly': dirty.assemblies[i]
                                });
                            }
...
//                                            console.log("Calling sendObjectInfo from game.tickAssemblies");
                ...
                /****************************** ASSEMBLY FAILED *********************************/
                    ...
                    if (helper.notFalse(player_socket)) {
                        ...
                        player_socket.emit('chat', {
                            'message': dirty.object_types[assembled_object_type_index].name +
        ...
        for (let decaying_floor_type of decaying_floor_types) {
            ...
            if (decaying_planet_coords.length > 0) {
                for (let planet_coord of decaying_planet_coords) {
                    ...
                    await main.updateCoordGeneric({}, {
                        'planet_coord_index': planet_coord_index,
                        'floor_type_id': new_floor_type_id
                    });
                }
        ...
        Object.keys(io.sockets.sockets).forEach(await async function (id) {
            ...
            try {
                ...
                if (room === "galaxy") {
                    ...
                    if (hp_effect < 0) {
                        socket.emit('damaged_data', {
                            'object_id': dirty.objects[ship_index].id,
                            'damage_amount': Math.abs(hp_effect),
                            'was_damaged_type': 'hp',
                        ...
                        socket.emit('damaged_data', {
                            'object_id': dirty.objects[player_index].id,
                            'damage_amount': hp_effect,
                            'was_damaged_type': 'hp',
                            'damage_source_type': 'floor',
                            'damage_types': ['healing']
                        });
...
async function processAutopilot(dirty, autopilot) {
    try {
        ...
        let autopilot_index = dirty.autopilots.findIndex(function (obj) {
            return obj && obj.id === autopilot.id;
        });
        ...
        let destination_coord_index = await main.getCoordIndex({
            'tile_x': autopilot.destination_tile_x,
            'tile_y': autopilot.destination_tile_y
        });
...
async function tickStructure(dirty, npc_index) {
    ...
    try {
        ...
        let structure_index = dirty.structure_types.findIndex(function (obj) {
            return obj && obj.id === dirty.npcs[npc_index].current_structure_type_id;
        });
                        ...
                        if (await game_object.canPlace(dirty, 'planet', dirty.planet_coords[checking_coord_index],
                            ...
                            if (dirty.planet_coords[checking_coord_index].object_id) {
                                ...
                                let battle_linker_data = {
                                    ...
                                    'attacking_type': 'npc',
                                    'being_attacked_id': dirty.planet_coords[checking_coord_index].object_id,
                                ...
                                if (spawn_linker_index === -1) {
                                    ...
                                    if (object_type_index !== -1) {
                                        ...
                                        if (vending_machine_object_index !== -1) {
                                            ...
                                            let adding_to_data = {
                                                'adding_to_type': 'object',
                                                'adding_to_id': dirty.objects[vending_machine_object_index].id,
                                                'object_type_id': dirty.spawn_linkers[spawn_linker_index].spawns_object_type_id,
    ...
    try {
        ...
        for (let i = 0; i < dirty.researches.length; i++) {
            if (dirty.researches[i]) {
                ...
                let object_type_index = dirty.object_types.findIndex(function (obj) {
                    return obj && obj.id === dirty.researches[i].being_researched_object_type_id;
                });
                ...
                if (dirty.researches[i].current_tick_count < dirty.object_types[object_type_index].research_tick_count) {
                    ...
                    let research_level = await player.getLevel(dirty, {
                        'player_index': player_index, 'skill_type': 'researching',
                    ...
                    if (success) {
                        ...
                        if (helper.notFalse(player_socket)) {
                            ...
                            player_socket.emit('chat', {
                                'message': dirty.object_types[object_type_index].name + " Research completed successfully",
                                'scope': 'system'
                            });
                        ...
                        if (helper.notFalse(player_socket)) {
                            ...
                            player_socket.emit('chat', {
                                'message': dirty.object_types[object_type_index].name +
...
async function processSpawner(dirty, spawner, spawner_index, spawner_type) {
    try {
        ...
        if (spawner.current_spawn_linker_id !== 0) {
            let spawn_linker_index = dirty.spawn_linkers.findIndex(function (obj) {
                return obj && obj.id === spawner.current_spawn_linker_id;
            });
                        ...
                        if (planet_coord_index !== -1) {
                            ...
                            if (planet_index !== -1) {
                                ...
                                if (trap_linkers.length > 0) {
                                    ...
                                    let adding_to_data = {
                                        'adding_to_type': 'object', 'adding_to_id': trap_object.id,
...
async function pickUp(socket, dirty, data) {
    ...
    try {
        ...
        if (object_index !== -1 && dirty.objects[object_index].has_spawned_object) {
            ...
            let adding_to_data = {
                'adding_to_type': 'player',
                'adding_to_id': dirty.players[socket.player_index].id,
                'object_type_id': dirty.spawn_linkers[spawn_linker_index].spawns_object_type_id,
                'amount': dirty.spawn_linkers[spawn_linker_index].spawns_amount
            };
        ...
        if (monster_index !== -1 && dirty.monsters[monster_index].has_spawned_object) {
            ...
            let spawn_linker_index = dirty.spawn_linkers.findIndex(function (obj) {
                return obj && obj.id === dirty.monsters[monster_index].current_spawn_linker_id;
            });
            ...
            let player_farming_level = await player.getLevel(dirty, {
                'player_index': socket.player_index,
                'skill_type': 'farming'
            });
            ...
            if (rand < attack_chance) {
                ...
                let existing_battle_linker_index = dirty.battle_linkers.findIndex(function (obj) {
                    return obj &&
                ...
                if (existing_battle_linker_index === -1) {
                    let battle_linker_data = {
                        'id': uuidv1(),
                        'attacking_id': dirty.monsters[monster_index].id,
                        'attacking_type': 'monster',
                        'being_attacked_id': dirty.players[socket.player_index].id,
        ...
        if (object_index !== -1 && dirty.object_types[object_type_index].can_pick_up) {
            ...
            if (player_info.scope === 'planet') {
                await main.updateCoordGeneric(socket, {
                    'planet_coord_index': picking_up_coord_index,
                    'object_id': false
                });
        ...
        if (player_info.scope === 'planet') {
            ...
            await main.updateCoordGeneric(socket, {
                'ship_coord_index': picking_up_coord_index,
                'object_type_id': false
            });
        }
...
async function plant(socket, dirty, data) {
    ...
    try {
        ...
        let remove_inventory_data = {
            'player_index': socket.player_index, 'inventory_item_id': dirty.inventory_items[inventory_item_index].id,
            'amount': 1
        };
        await inventory.removeFromInventory(socket, dirty, remove_inventory_data);
...
async function repair(socket, dirty, data) {
    try {
        ...
        if (dirty.players[socket.player_index].planet_coord_id) {
            ...
            let repairing_index = dirty.repairing_linkers.findIndex(function (obj) {
                return obj
...
async function repairStop(socket, dirty, data) {
    try {
        let repairing_linker_index = dirty.repairing_linkers.findIndex(function (obj) {
            return obj && obj.player_id === socket.player_id &&
        ...
        socket.emit('repairing_linker_info', {
            'remove': true,
            'repairing_linker': dirty.repairing_linkers[repairing_linker_index]
        });
    ...
    try {
        ...
        for (let i = 0; i < dirty.assembly_linkers.length; i++) {
            ...
            if (type === 'object_type' && dirty.assembly_linkers[i].required_for_object_type_id &&
                ...
                let remove_inventory_data = {
                    'player_index': socket.player_index,
                    'removing_object_type_id': dirty.assembly_linkers[i].object_type_id,
    ...
    try {
        ...
        if (dirty.players[player_index].planet_coord_id) {
            ...
            if (planet_coord_index === -1 && dirty.planet_coords[player_planet_coord_index].level > 0) {
                ...
                let below_planet_coord_data = {
                    'planet_id': dirty.planet_coords[player_planet_coord_index].planet_id, 'planet_level': below_level,
                    'tile_x': x, 'tile_y': y
                };
        ...
        if (temp_coord.floor_type_id) {
            let floor_type_index = dirty.floor_types.findIndex(function (obj) {
                return obj && obj.id === temp_coord.floor_type_id;
            });
                ...
                for (let ai_rule of ai_rules) {
                    ...
                    if (ai_rule.rule === 'floor_no_others') {
                        if (socket.player_id !== dirty.objects[ai_index].player_id) {
                            ...
                            socket.emit('chat', {
                                'message': "The AI does not allow you replace floors here",
                                'scope': 'system'
                            });
                            return false;
                        }
    ...
    try {
        ...
        if (data.coord_id) {
            ...
            if (coord_index !== -1) {
                await main.updateCoordGeneric(socket, {
                    'coord_index': coord_index,
                    'floor_type_id': data.floor_type_id
                });
            }
...
async function research(socket, dirty, data) {
    ...
    try {
        ...
        let existing_research_index = dirty.researches.findIndex(function (obj) {
            return obj &&
    ...
    try {
        for (let i = 0; i < dirty.assembly_linkers.length; i++) {
            ...
            if (assembly_linker.required_for_object_type_id) {
                socket.emit('object_type_assembly_linker_data', {
                    'id': assembly_linker.id,
                    'required_for_object_type_id': assembly_linker.required_for_object_type_id,
                    'object_type_id': assembly_linker.object_type_id,
                    'amount': assembly_linker.amount
                });
            } else if (assembly_linker.required_for_floor_type_id) {
                socket.emit('floor_type_assembly_linker_data', {
                    'id': assembly_linker.id, 'required_for_floor_type_id': assembly_linker.required_for_floor_type_id,
...
function sendObjectTypeConversionLinkerData(socket, dirty) {
    ...
    dirty.object_type_conversion_linkers.forEach(function (conversion_linker, i) {
        socket.emit('object_type_conversion_linker_info', {
            'object_type_conversion_linker':
                {
...
async function sendPlayerStats(socket, dirty) {
    ...
    try {
        ...
        socket.emit('player_stats', {
            ...
            'exp': socket.player_exp,
            'level': socket.player_level,
            'attack': socket.player_attack,
            'defense': socket.player_defense,
            'player_hp': socket.player_current_hp,
            'max_hp': socket.player_max_hp,
            'energy': socket.player_energy,
            'is_admin': socket.is_admin,
            'farming_skill_points': socket.farming_skill_points,
            'cooking_skill_points': dirty.players[socket.player_index].cooking_skill_points,
            'manufacturing_skill_points': dirty.players[socket.player_index].manufacturing_skill_points,
            'laser_skill_points': dirty.players[socket.player_index].laser_skill_points,
...
async function switchBody(socket, dirty, new_body_id, move_inventory) {
    try {
        ...
        if (dirty.players[player_index].planet_coord_id) {
            ...
            await main.updateCoordGeneric(socket, {
                'planet_coord_index': old_player_coord_index,
                'player_id': false,
                'object_id': old_body_id,
                'object_type_id': dirty.objects[old_body_index].object_type_id
            });

            // update the new planet coord
            await main.updateCoordGeneric(socket, {
                'planet_coord_index': new_body_coord_index,
            ...
            await main.updateCoordGeneric(socket, {
                'ship_coord_index': old_player_coord_index,
                'player_id': false,
                'object_id': old_body_id,
                'object_type_id': dirty.objects[old_body_index].object_type_id
            });
            ...
            await main.updateCoordGeneric(socket, {
                'ship_coord_index': new_body_coord_index,
                'player_id': dirty.players[player_index].id, 'object_id': false
            });
    ...
    try {
            ...
            for (let y = elevator_info.coord.tile_y - 2; y <= elevator_info.coord.tile_y + 2; y++) {
                ...
                if (found_coord_index === -1) {
                    ...
                    if (dirty.objects[elevator_index].planet_coord_id) {
                        ...
                        let possible_coord_index = await main.getPlanetCoordIndex({
                            'planet_id': elevator_info.coord.planet_id,
                        ...
                        let possible_coord_index = await main.getShipCoordIndex({
                            'ship_id': elevator_info.coord.ship_id,
        ...
        if (found_coord_index !== -1) {
            if (elevator_info.coord.planet_id) {
                await main.updateCoordGeneric(socket, {
                    'planet_coord_index': player_info.coord_index,
                    'player_id': false
                });
                await main.updateCoordGeneric(socket, {
                    'planet_coord_index': found_coord_index,
                ...
                await main.updateCoordGeneric(socket, {
                    'ship_coord_index': player_info.coord_index,
                    'player_id': false
                });
                await main.updateCoordGeneric(socket, {
                    'ship_coord_index': found_coord_index,
