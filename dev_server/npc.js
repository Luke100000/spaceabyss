...
async function bugAttackCode(dirty, npc_index) {
    try {
        ...
        if (!dirty.npcs[npc_index].planet_coord_id) {
            ...
            let chosen_planet_coord_index = dirty.coords.findIndex(function (obj) {
                return obj &&
            ...
            dirty.npc_tasks.push({
                'npc_id': dirty.npcs[npc_index].id, 'destination_tile_x': destination_tile_x,
    ...
    try {
        ...
        dirty.inventory_items.forEach(function (inventory_item, i) {
            if (inventory_item.npc_id === npc_id) {
                inventory.removeFromInventory(false, dirty, {
...
async function foragerSetTask(dirty, npc_index, debug_npc_ids) {
    try {
        ...
        if (!dirty.npcs[npc_index].planet_coord_id) {
            ...
            dirty.npc_tasks.push({
                'npc_id': dirty.npcs[npc_index].id,
                'destination_tile_x': dirty.coords[chosen_planet_coord_index].tile_x,
                'destination_tile_y': dirty.coords[chosen_planet_coord_index].tile_y,
                'destination_planet_id': chosen_planet.id
            });
...
async function getIndex(dirty, npc_id) {
    try {
        ...
        let npc_index = dirty.npcs.findIndex(function (obj) {
            return obj && obj.id === npc_id;
        });
        ...
        if (rows[0]) {
            for (let i = 0; i < rows.length; i++) {
                ...
                let ii_index = dirty.inventory_items.findIndex(function (obj) {
                    return obj && obj.id === parseInt(inventory_item.id)
                });
                if (ii_index === -1) {
    ...
    try {
        ...
        dirty.npcs.forEach(function (npc, i) {
            try {
                ...
                let task_index = dirty.npc_tasks.findIndex(function (obj) {
                    return obj && obj.npc_id === npc.id;
                });
                ...
                if (dirty.npcs[i].current_hp < dirty.npcs[i].max_hp) {
                    ...
                    let existing_eating_task_index = dirty.npc_tasks.findIndex(function (obj) {
                        return obj && obj.npc_id === dirty.npcs[i].id &&
                ...
                if (dirty.npcs[i].current_hp * 2 < dirty.npcs[i].max_hp) {
                    let existing_escape_task_index = dirty.npc_tasks.findIndex(function (obj) {
                        return obj && obj.npc_id === dirty.npcs[i].id &&
...
async function npcPickUp(dirty, npc_index, coord_index) {
    ...
    try {
        ...
        if (object_index !== -1 && dirty.objects[object_index].has_spawned_object) {
            ...
            let adding_to_data = {
                'adding_to_type': 'npc',
                'adding_to_id': dirty.npcs[npc_index].id,
            ...
            if (dirty.planet_coords[coord_index].object_id) {
                ...
                let adding_to_data = {
                    'adding_to_type': 'npc',
                    'adding_to_id': dirty.npcs[npc_index].id,
                    'object_type_id': dirty.planet_coords[coord_index].object_type_id,
                    'amount': dirty.planet_coords[coord_index].object_amount
                };
            ...
            if (dirty.npcs[npc_index].coord_id) {
                ...
                if (dirty.npc_tasks[task_index].path && dirty.npc_tasks[task_index].path.length > 0) {
                    ...
                    if (next_coord_index !== -1) {
                        possible_next_moves.push({
                            'coord': dirty.coords[next_coord_index],
                ...
                if (!found_move) {
                    ...
                    worker_data = {
                        'coords': pathfinding_coords,
                        'npc': dirty.npcs[npc_index],
                        'origin_x': dirty.coords[npc_coord_index].tile_x,
                        'origin_y': dirty.coords[npc_coord_index].tile_y,
                        'destination_x': dirty.npc_tasks[task_index].destination_tile_x,
                        'destination_y': dirty.npc_tasks[task_index].destination_tile_y
                    };
                ...
                if (dirty.npc_tasks[task_index].destination_planet_id) {
                    ...
                    if (x_difference !== 0) {
                        ...
                        next_coord_index = await main.getPlanetCoordIndex({
                            'planet_id': dirty.planet_coords[npc_coord_index].planet_id,
                        ...
                        if (next_coord_index !== -1) {
                            possible_next_moves.push({
                                'coord': dirty.planet_coords[next_coord_index],
                                'coord_index': next_coord_index
                            });
                        }
                    }
...
async function setNpcStructure(dirty, npc_index) {
    ...
    let planet_index = await planet.getIndex(dirty, {
        'planet_id': dirty.npcs[npc_index].planet_id,
        'source': 'game.setNpcStructure'
    });
            ...
            if (dirty.npcs[npc_index].dream_job_id) {
                ...
                if (job_requirement_linkers.length > 0) {
                    for (let requirement_linker of job_requirement_linkers) {
                        let inventory_index = dirty.inventory_items.findIndex(function (obj) {
                            return obj &&
    ...
    try {
        ...
        for (let i = 0; i < dirty.npcs.length; i++) {
            ...
            if (dirty.npcs[i] && dirty.npcs[i].enslaved_to_npc_id === dirty.npcs[npc_index].id) {
                ...
                let enslaved_task_index = dirty.npc_tasks.findIndex(function (obj) {
                    return obj && obj.npc_id === dirty.npcs[i].id
                });
                ...
                if (enslaved_task_index === -1) {
                    ...
                    if (need_to_send_to_slaver_planet) {
                        let slaver_planet_index = dirty.planets.findIndex(function (obj) {
                            return obj && obj.planet_type_id === 31;
                        });
                        if (slaver_planet_index !== -1) {
                            ...
                            dirty.npc_tasks.push({
                                'npc_id': dirty.npcs[npc_index].id,
                                'destination_tile_x': destination_tile_x,
                                'destination_tile_y': destination_tile_y,
                                'destination_planet_id': dirty.planets[slaver_planet_index].id
                            });
                        ...
                        if (planet_coord_index !== -1) {
                            if (dirty.planet_coords[planet_coord_index].npc_id && dirty.planet_coords[planet_coord_index].npc_id !== dirty.npcs[npc_index].id) {
                                ...
                                if (being_enslaved_npc_index !== -1) {
                                    world.enslave(false, dirty, {
                                        'slaver_npc_index': npc_index,
                                        'being_enslaved_npc_index': being_enslaved_npc_index
                                    });
                                }
...
async function spawn(dirty, data = false) {
    try {
        ...
        if (!data.npc_job_id) {
            ...
            let starter_job_index = dirty.npc_jobs.findIndex(function (obj) {
                return obj && obj.id === parseInt(data.npc_job_id);
            });
        ...
        let structure_linker_index = dirty.job_linkers.findIndex(function (obj) {
            return obj && obj.job_id === starter_job.id &&
        ...
        if (starter_job.id === 5) {
            ...
            await world.addRule(false, dirty, {
                'object_id': dirty.objects[new_ship_index].id,
                'new_rule': 'attack_all_players'
            });
        ...
        if (met_all_requirements) {
            ...
            if (can_build_result === true) {
                ...
                await game.buildStructure(dirty, {
                    'npc_index': npc_index,
                    'structure_type_id': dirty.npcs[npc_index].current_structure_type_id
                });
