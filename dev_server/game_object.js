...
async function canPlace(dirty, scope, coord, data) {
    try {
        ...
        let object_type_index = dirty.object_types.findIndex(function (obj) {
            return obj && obj.id === object_type_id;
        });
        ...
        for (let x = origin_tile_x - tiles_around; placing_coord_index === -1 && x <= origin_tile_x + tiles_around; x++) {
            for (let y = origin_tile_y - tiles_around; placing_coord_index === -1 && y <= origin_tile_y + tiles_around; y++) {
                ...
                if (data.planet_coord_index) {
                    trying_coord_index = await main.getPlanetCoordIndex({
                ...
                if (data.ship_coord_index) {
                    trying_coord_index = await main.getShipCoordIndex({
...
async function damage(dirty, object_index, damage_amount, data) {
    try {
        ...
        if (new_object_hp <= 0) {
            ...
            if (data.battle_linker) {
                await deleteObject(dirty, {
                    'object_index': object_index,
                    'reason': 'battle',
                    'reason_type': 'player',
                    'reason_id': data.battle_linker.attacking_id
                });
            } else {
...
async function deleteObject(dirty, data) {
    ...
    try {
        ...
        let object_type_index = dirty.object_types.findIndex(function (obj) {
            return obj && obj.id === dirty.objects[data.object_index].object_type_id;
        });
                ...
                object_type_display_linkers.forEach(await async function (linker) {
                    ...
                    if (linker.position_x === 0 && linker.position_y === 0) {
                        ...
                        if (object_info.scope === 'galaxy') {
                            let belongs_coord_index = await main.getCoordIndex({
                                'tile_x': belongs_x,
                                'tile_y': belongs_y
                            });
                            if (belongs_coord_index !== -1) {
                                await main.updateCoordGeneric(false, {
                                    'coord_index': belongs_coord_index,
                                    'belongs_to_object_id': false
                                });
                            }
                        } else if (object_info.scope === 'planet') {
                            let belongs_coord_index = await main.getPlanetCoordIndex({
                                'planet_id': object_info.coord.planet_id,
                                'planet_level': object_info.coord.level, 'tile_x': belongs_x, 'tile_y': belongs_y
                            });
                            if (belongs_coord_index !== -1) {
                                await main.updateCoordGeneric(false, {
                                    'planet_coord_index': belongs_coord_index,
                                    'belongs_to_object_id': false
                                });
                            }
                        } else if (object_info.scope === 'ship') {
                            let belongs_coord_index = await main.getShipCoordIndex({
                                'ship_id': object_info.coord.ship_id,
                                'level': object_info.coord.level,
                                'tile_x': belongs_x, 'tile_y': belongs_y
                            });
                            if (belongs_coord_index !== -1) {
                                await main.updateCoordGeneric(false, {
                                    'ship_coord_index': belongs_coord_index,
                                    'belongs_to_object_id': false
                                });
                            }
                        }
        ...
        if (dirty.objects[data.object_index].object_type_id === 72) {
            ...
            if (dirty.objects[data.object_index].planet_coord_id) {
                let planet_index = await planet.getIndex(dirty, {
                    'planet_id': dirty.objects[data.object_index].planet_id,
                    'source': 'game_object.deleteObject'
                });
                if (planet_index !== -1) {
        ...
        for (let inventory_item of dirty.inventory_items) {
            if (inventory_item && dirty.objects[data.object_index] && inventory_item.owned_by_object_id === dirty.objects[data.object_index].id) {
                await inventory.removeFromInventory(false, dirty, {
...
async function getIndex(dirty, object_id) {
    try {
        ...
        let object_index = dirty.objects.findIndex(function (obj) {
        ...
        if (object_index === -1) {
            ...
            if (rows[0]) {
                ...
                let memory_index = dirty.objects.findIndex(function (obj) {
                    return obj && obj.id === object_id;
                });
                if (memory_index !== -1) {
                ...
                let planet_coord_index = dirty.planet_coords.findIndex(function (obj) {
                    return obj && obj.object_id === object_id;
                });
                if (planet_coord_index !== -1) {
...
async function getShipCurrentCoords(socket, dirty, display_linkers) {
    ...
    try {
        ...
        for (let i = 0; i < display_linkers.length; i++) {
            ...
            dirty.objects[dirty.players[socket.player_index].ship_index].current_coords.push({
...
async function place(socket, dirty, data) {
    ...
    try {
        ...
        // HOLE
            ...
            let below_data = {
                'planet_id': dirty.planet_coords[data.planet_coord_index].planet_id, 'planet_level': below_level,
            ...
            if (!already_have_stairs) {
                ...
                await main.updateCoordGeneric(socket, {
                    'planet_coord_index': below_coord_index,
        ...
        // STAIRS
            ...
            let above_data = {
                'planet_id': dirty.planet_coords[data.planet_coord_index].planet_id, 'planet_level': above_level,
                'tile_x': dirty.planet_coords[data.planet_coord_index].tile_x,
                'tile_y': dirty.planet_coords[data.planet_coord_index].tile_y
            };
            ...
            let new_object_index = await world.insertObjectType(socket, dirty, inserting_object_type_data);
            await main.updateCoordGeneric(socket, {
                'planet_coord_index': above_coord_index,
                'object_index': new_object_index
            });
    ...
    try {
        ...
        if (!has_display_linkers) {
            ...
            for (let i = 0; i < display_linkers.length; i++) {
                ...
                if (scope === 'galaxy') {
                    let update_coord_index = await main.getCoordIndex({
                        'tile_x': linker_tile_x,
                        'tile_y': linker_tile_y
                    });
                    ...
                    let update_coord_index = await main.getShipCoordIndex({
                        'ship_id': dirty.ship_coords[origin_coord_index].ship_id,
                        'level': dirty.ship_coords[origin_coord_index].level,
                        'tile_x': linker_tile_x,
                        'tile_y': linker_tile_y
                    });
                    if (update_coord_index !== -1) {
                    ...
                    let update_coord_index = await main.getPlanetCoordIndex({
                        'planet_id': dirty.planet_coords[origin_coord_index].planet_id,
                        'planet_level': dirty.planet_coords[origin_coord_index].level,
                        'tile_x': linker_tile_x,
                        'tile_y': linker_tile_y
                    });
        ...
        if (object_type_spawn_linkers.length > 0) {
            ...
            if (met_requirements) {
                ...
                // We finish it
                    ...
                    await game.finishSpawnLinker(dirty, {
                        'object_index': object_index,
                    ...
                    for (let i = 0; i < dirty.ship_linkers.length; i++) {
                        if (dirty.ship_linkers[i] && dirty.ship_linkers[i].ship_type_id === dirty.object_types[object_type_index].id) {
                            ...
                            let ship_coord_index = await main.getShipCoordIndex({
                                'ship_id': dirty.objects[s].id, 'level': dirty.ship_linkers[i].level,
