    ...
    try {
        ...
        if (being_damaged_monster_index !== -1) {
            ...
            if (ai_index !== -1) {
                ...
                io.to(room).emit('hp_change_data',
                    {
                        'monster_id': dirty.monsters[being_damaged_monster_index].id,
                        'damage_amount': data.damage_amount,
                        'was_damaged_type': 'energy',
        ...
        if (being_damaged_player_index !== -1) {
            ...
            if (ai_index !== -1) {
                ...
                io.to(room).emit('hp_change_data',
                    {
                        'player_id': dirty.players[being_damaged_player_index].id,
                        'damage_amount': data.damage_amount,
                        'was_damaged_type': 'energy',
                        'damage_source_type': 'player',
                        'damage_source_id': data.battle_linker.attacking_id
            ...
            await player.damage(dirty, {
                'player_index': being_damaged_player_index,
        ...
        if (nothing_is_damaged) {
            ...
            if (data.planet_coord_index) {
                io.to(room).emit('damaged_data', {
                    'planet_coord_id': dirty.planet_coords[data.planet_coord_index].id,
                    'damage_amount': 0,
                    'damage_types': [data.damage_type]
                });
            } else if (data.ship_coord_index) {
                io.to(room).emit('damaged_data', {
                    'ship_coord_id': dirty.ship_coords[data.ship_coord_index].id,
                    'damage_amount': 0,
                    'damage_types': [data.damage_type]
                });
            }
        ...
        if (player_attack_profile.decrement_equipment_linker_ids.length > 0) {
            ...
            for (let i = 0; i < player_attack_profile.decrement_equipment_linker_ids.length; i++) {
                ...
                if (equipment_linker_index !== -1) {
                    ...
                    removeFromEquipmentLinker(dirty, io.sockets.connected[player_socket_id], {
                        'equipment_linker_index': equipment_linker_index,
                        'amount': 1
                    });
            ...
            io.to(object_info.room).emit('damaged_data',
                {
                    ...
                    'was_damaged_type': 'energy',
                    'damage_source_type': 'object',
                    'damage_source_id': dirty.monsters[monster_index].id,
            ...
            io.to(player_info.room).emit('damaged_data',
                {
                    ...
                    'calculating_range': calculating_range,
                    'flavor_text': flavor_text,
                    'additional_effect': monster_attack.additional_effect
                });
        } else {
            await player.damage(dirty, {
                'player_index': player_index,
                'damage_amount': damage_amount,
                'battle_linker': battle_linker,
                'damage_types': [monster_attack.damage_type],
                'calculating_range': calculating_range,
                'flavor_text': flavor_text,
                'damage_effect': monster_attack.damage_effect
    ...
    try {
        ...
        for (let battle_linker of monster_battle_linkers) {
            ...
            if (global.battle_time % dirty.monster_types[monster_type_index].attack_movement_delay === 0) {
                await monster.move(dirty, monster_index, {
                    'reason': 'battle', 'battle_linker': battle_linker,
            ...
            io.to(monster_info.room).emit('damaged_data',
                {
                    ...
                    'damage_source_id': dirty.npcs[npc_index].id,
                    'calculating_range': calculating_range
                });
    ...
    try {
        ...
        if (attack === 0) {
            ...
            if (battle_linker.socket_id) {
                io.to(battle_linker.socket_id).emit('result_info', {
                    'status': 'failure',
                    'text': 'No attack power. Do your weapons have energy/material?'
                });
            }
        ...
        if (shielded) {
            ...
            io.to(defending_object_info.room).emit('damaged_data',
                {
                    'object_id': dirty.objects[defending_object_index].id,
                    'damage_amount': damage_amount,
                    'was_damaged_type': 'energy',
                    'damage_source_type': 'object',
                    'damage_source_id': dirty.objects[attacking_object_index].id,
                    'calculating_range': calculating_range,
                    'damage_types': object_attack_profile.damage_types
                });
        ...
        // Adding in code for a spacelane beacon to help out
            ...
            if (watching_object_index !== -1 && dirty.objects[watching_object_index].energy > damage_amount) {
                ...
                objectAttackObject(dirty, false, {
                    'attacking_object_index': watching_object_index,
                    'defending_object_index': attacking_object_index
                });
            }
        ...
        await game_object.damage(dirty, defending_object_index, damage_amount, {
            'battle_linker': battle_linker,
            'object_info': defending_object_info,
            'calculating_range': calculating_range,
            'damage_types': object_attack_profile.damage_types,
            'damage_source_type': 'object',
            'damage_source_id': dirty.objects[attacking_object_index].id
...
async function objectAttackPlanet(dirty, battle_linker, data = false) {
    try {
        ...
        */
        ...
        if (ai_index !== -1) {
            ...
            io.to(defending_planet_info.room).emit('damaged_data',
                {
                    'planet_id': dirty.planets[defending_planet_index].id,
                    'damage_amount': damage_amount,
                    'was_damaged_type': 'energy',
                    'damage_source_type': 'object',
                    'damage_source_id': dirty.objects[attacking_object_index].id,
        ...
        */
        ...
        // No AI - Do the attack/defense normally
        await game.damagePlanet(dirty, {
            'planet_index': defending_planet_index,
            'damage_amount': damage_amount,
            'battle_linker': battle_linker,
            'planet_info': defending_planet_info,
            'calculating_range': calculating_range,
            'damage_types': object_attack_profile.damage_types,
            'damage_source_type': 'object',
            'damage_source_id': dirty.objects[attacking_object_index].id
...
async function objectAttackPlayer(dirty, battle_linker, data = false) {
    ...
    try {
            ...
            io.to(player_info.room).emit('damaged_data',
                {
                    ...
                    'was_damaged_type': 'energy',
                    'damage_source_type': 'object',
                    'damage_source_id': dirty.objects[object_index].id,
                    'damage_types': object_attack_profile.damage_types,
                    'calculating_range': calculating_range
                });
        ...
        await player.damage(dirty, {
            'player_index': player_index, 'damage_amount': damage_amount,
            'damage_types': object_attack_profile.damage_types,
            ...
            io.to(monster_info.room).emit('damaged_data',
                {
                    ...
                    'was_damaged_type': 'energy',
                    'damage_source_type': 'player',
                    'damage_source_id': dirty.players[player_index].id,
                    'calculating_range': calculating_range
                });
                        ...
                        if (x === monster_info.coord.tile_x && y === monster_info.coord.tile_y) {
                            ...
                            if (monster_info.scope === 'planet') {
                                ...
                                let ship_coord_index = await main.getShipCoordIndex({
                                    'ship_id': monster_info.coord.ship_id,
                                    'level': monster_info.coord.level,
                                    'tile_x': x,
                                    'tile_y': y
                                });
...
async function playerAttackNpc(dirty, battle_linker) {
    ...
    try {
        ...
        if (try_enslavement) {
            world.enslave(io.sockets.connected[battle_linker.socket_id], dirty, {
                'enslaving_player_index': player_index,
                'being_enslaved_npc_index': npc_index
            });
        }
...
async function playerAttackPlayer(dirty, battle_linker) {
    ...
    try {
        ...
        if (ai_index !== -1) {
            ...
            io.to(defending_player_info.room).emit('damaged_data',
                {
                    'player_id': dirty.players[defending_player_index].id,
                    'damage_amount': damage_amount,
                    'was_damaged_type': 'energy',
                    'damage_source_type': 'player',
                    'damage_source_id': dirty.players[attacking_player_index].id,
                    'damage_types': player_attack_profile.damage_types,
                    'calculating_range': calculating_range
                });
                ...
                io.to(defending_player_info.room).emit('damaged_data',
                    {
                        ...
                        'damage_source_id': dirty.players[attacking_player_index].id,
                        'calculating_range': calculating_range,
                        'damage_types': [dirty.object_types[watching_object_type_index].equip_skill]
                    });
                objectAttackPlayer(dirty, false, {
                    'object_index': watching_object_index,
                    'player_index': attacking_player_index
                });
        ...
        await player.damage(dirty, {
            'player_index': defending_player_index, 'damage_amount': damage_amount,
            'battle_linker': battle_linker, 'calculating_range': calculating_range
        });
...
async function removeFromEquipmentLinker(dirty, socket, data) {
    try {
        ...
        socket.emit('equipment_linker_info', {
            'remove': true,
            'equipment_linker': dirty.equipment_linkers[data.equipment_linker_index]
        });
